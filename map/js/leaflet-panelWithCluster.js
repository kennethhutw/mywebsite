...
L.Control.PanelLayers = L.Control.Layers.extend({
	...
	initialize : function(baseLayers,overlays,options){
		...
		for (i in overlays)
		{
			...
			else if(overlays[i].layer && overlays[i].Cluster){
				if(overlays[i].hasOwnProperty("marker") && overlays[i].marker.hasOwnProperty("active"))
				{
					IsActiveMarker =overlays[i].marker.active;
				}
				this._addClusterLayer(overlays[i], true,false);
			}
			else	
			{	
				...
				this._addLayer(overlays[i], true, null,IsActiveMarker);
			}
		}
	},
	...
	_addClusterLayer: function (layer, overlay, IsActiveMarker){
		var id = L.stamp(layer),markertype,displayname;
		if(layer.active)
			this._layersActives.push(layer.layer);
	...
	_createItem: function(obj) {
		...
		if (obj.overlay) {
			...
			if(obj.markertype=="Cluster")
			{
				checked = this._map.hasLayer(obj.layer.layer);
				input.defaultChecked = checked;
				L.DomEvent.on(input, 'click', this._onClusterInputClick, this);
			}else{
				input.defaultChecked = checked;
	...
	_onClusterInputClick: function(e){
		var i, input, obj,
		...
		{
			if (input.checked && !this._map.hasLayer(obj.layer)) {
				this._map.addLayer(obj.layer.layer);
			}
			else{
				this._map.removeLayer(obj.layer.layer);
			}
		}
		this._handlingClick = false;
